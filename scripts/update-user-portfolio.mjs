...
const projectUpdates = {
  "Folioframe — 직군 맞춤형 웹 포트폴리오 서비스": {
    summary: "개발자가 기술 선택과 문제 해결 과정을 구조적으로 기록하고 하나의 링크로 발행할 수 있는 웹 서비스를 개발했습니다.",
    role: "정보 구조 설계, Next.js 화면 구현, PostgreSQL 데이터 모델링, 인증과 Vercel 배포를 담당했습니다.",
    problem: "개발 프로젝트 설명이 기술 목록과 결과 화면에 치우쳐 면접관이 구현 판단과 실제 기여를 파악하기 어려운 문제가 있었습니다.",
    troubleshooting: "입력 상태를 프로필·프로젝트 단위로 분리하고, 서버 검증과 미리보기 데이터를 공통 계약 타입으로 연결했습니다.",
    result: "프로필 작성부터 프로젝트 편집, 테마 선택, 공개 발행까지 단일 데이터 흐름으로 연결하여 실제 서비스로 배포·운영 중입니다.",
    target_audience: "기술 선택과 문제 해결 과정을 정리해 취업 포트폴리오로 발행하려는 개발자",
    goal: "개발자의 기여 범위와 구현 판단을 면접관이 빠르게 이해할 수 있는 작성·발행 경험 구축",
    constraints: "로그인 사용자 데이터와 공개 페이지를 철저히 분리하면서도 미리보기와 실제 발행 화면이 동일해야 했습니다.",
    key_decision: "Next.js App Router와 Server Actions/API Route 분리 설계를 채택해 렌더링 성능과 데이터 정합성을 동시에 확보했습니다.",
    collaboration: "1인 풀스택 개발 프로젝트로 기획부터 배포, 사용자 피드백 수렴 및 지속적 기능 개선을 전담했습니다.",
    ...
    learnings: "데이터 구조 설계 시 미리보기와 실제 공개 뷰의 타입 일관성이 개발 생산성에 미치는 영향을 체감했습니다.",
    next_time: "향후 방문자 통계 분석 대시보드와 커스텀 도메인 연결 기능을 추가로 고도화할 계획입니다.",
  },
  "CapLog": {
    summary: "스크린샷 속 텍스트와 이미지를 On-Device OCR 및 Vision AI로 분석해 자동으로 카테고리화하고 관리하는 지능형 스크린샷 큐레이션 서비스입니다.",
    role: "iOS 앱에서 Photos·PhotoKit의 스크린샷을 읽고 Apple Vision OCR·이미지 분류로 분석하는 흐름과 Spring Boot API 연동을 담당했습니다. 개인정보 패턴 마스킹, AI 카드 생성, 폴더·검색·위치 기반 추천·알림, 친구·채팅·카드 공유, JWT 인증과 사용자별 데이터 격리를 구현했습니다. 이후에는 SwiftUI 화면 구조와 UI/UX를 재설계하고 Mock 기능을 실제 서버 데이터 흐름으로 전환했으며, 로컬 이미지 보호 저장과 AI 실패 시 대체 카드 처리까지 보완했습니다.",
    problem: "스크린샷이 사진첩에 무분별하게 쌓여 필요한 정보를 제때 찾지 못하고 개인정보 노출 위험이 존재하는 문제를 해결하고자 했습니다.",
    troubleshooting: "Apple Vision 프레임워크를 활용한 온디바이스 1차 전처리와 Spring Boot 비동기 AI 분석 파이프라인을 구축해 처리 속도를 대폭 단축했습니다.",
    result: "Apple Vision 기반 온디바이스 스크린샷 텍스트 인식과 Spring Boot API 연동을 완성하고, 개인정보 보호 마스킹과 카드 큐레이션 기능을 구현했습니다.",
