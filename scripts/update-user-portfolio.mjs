...
const projectsData = {
  ...
  "Ticker — Human Stock Market": {
    summary: `개인의 가치, 성장 지표, 성과를 주식 시장 메커니즘으로 시각화하고 상호 투자·응원할 수 있는 소셜 파이낸스 플랫폼입니다. KAIST 몰입캠프 협업 프로젝트로 개발되었으며, 실시간 호가 변동과 가상 주식 거래 시스템을 웹으로 구현했습니다.`,
  ...
  "Love Algorithm — 알고리즘보다 어려운 건 사랑이었다": {
    summary: `알고리즘 문제 해결 상황과 개발자 밈을 인터랙티브 스토리텔링과 연애 시뮬레이션 게임으로 유쾌하게 풀어낸 웹 콘텐츠입니다. KAIST 몰입캠프에서 팀원들과 함께 개발했습니다.`,
  ...
  "EGGO — 농꾸하고 작심삼일 타파하자": {
    summary: `지속적인 목표 달성 습관 형성을 돕기 위해 귀여운 농장 꾸미기 게이미피케이션과 온디바이스 AI 미션 인증을 결합한 스마트 습관 관리 모바일 앱입니다. KAIST 몰입캠프에서 팀 프로젝트로 개발했습니다.`,
  ...
  "Localhost — 멀티플레이어 뮤직 퀴즈 게임": {
    summary: `여러 사용자가 웹 브라우저를 통해 실시간으로 방에 접속해 음악을 함께 듣고 퀴즈를 맞히는 멀티플레이어 실시간 웹 게임입니다. KAIST 몰입캠프에서 팀 프로젝트로 개발했습니다.`,
...
console.log(`Updating ${projects.rows.length} projects in DB with long, comprehensive content...`);
...
for (const row of projects.rows) {
  ...
  if (patch) {
    ...
    console.log("Updated long content for:", row.title);
  }
}

await client.end();
console.log("All projects updated with deep, long original content!");
