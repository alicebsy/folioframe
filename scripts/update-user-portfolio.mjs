...
새로운 기술이나 언어를 배울 때는 AI를 적극 활용해 학습 속도를 극대화하고, 직접 코드를 작성하고 검증하며 단단한 제 것으로 체화하겠습니다. 백엔드 개발을 중심으로 성장하면서도 **사용자가 실제로 마주하는 흐름과 서비스 전체의 완성도를 함께 고민하는 개발자**로 성장하고자 합니다.`;
...
console.log("Successfully updated portfolio table with curated highlights!");

const projectsData = {
  "Folioframe — 직군 맞춤형 웹 포트폴리오 서비스": {
    summary: `개발자가 **기술 선택과 문제 해결 과정을 구조적으로 기록하고 하나의 링크로 발행**할 수 있는 웹 서비스를 개발했습니다. 지금 보고 계신 이 웹 포트폴리오 역시 Folioframe 서비스를 직접 기획하고 개발하여 발행한 실제 프로덕션 결과물입니다. 단순한 이력 나열을 넘어 기여도와 문제 해결 능력을 온전히 전달할 수 있도록 돕습니다.`,
    ...
    problem: `기존 노션이나 정적 블로그 기반 포트폴리오는 프로젝트 설명이 단순 기술 스택 나열이나 완성된 결과 화면 캡처에 치우쳐 있어, **채용 담당자와 면접관이 지원자의 실제 기여도와 트러블슈팅 과정, 기술적 판단 근거를 파악하기 어려운 문제**가 있었습니다. 또한 작성자 입장에서도 레이아웃을 통일성 있게 유지하고 모바일과 PC 환경에 맞추어 깔끔한 링크로 공유하기 번거로웠습니다.`,
    troubleshooting: `프로필 입력 상태와 프로젝트 데이터 구조를 분리하고, **서버 측 검증 스키마와 대시보드 실시간 미리보기 데이터를 공통 TypeScript 계약 타입으로 단일화**했습니다. Next.js App Router와 Server Actions/API Route 분리 설계를 채택하여 편집 시의 지연 시간을 최소화하고, Serverless Neon PostgreSQL과의 커넥션 풀링을 최적화해 데이터 정합성과 빠른 렌더링 성능을 동시에 확보했습니다.`,
    result: `프로필 작성부터 8개 상세 프로젝트 편집, 4가지 맞춤 테마 전환, 25MB 파일 첨부, 고유 슬러그 기반의 웹 포트폴리오 즉시 발행까지 **단일 데이터 흐름으로 이어지는 웹 빌더를 완성하여 실제 서비스로 배포·운영 중**입니다. (현재 보고 계신 이 포트폴리오도 Folioframe으로 직접 빌드하고 배포한 실제 라이브 서비스입니다.)`,
  ...
  "CapLog": {
    ...
    role: `iOS 앱에서 Photos·PhotoKit을 통한 스크린샷 자동 탐색 및 **Apple Vision Framework 기반 온디바이스 OCR과 이미지 분류 파이프라인 구축, Spring Boot 백엔드 REST API 설계**를 담당했습니다. 개인정보 정규식 마스킹, AI 요약 카드 생성, 스마트 폴더 분류, 텍스트 검색, CoreLocation 기반 위치 추천 및 마감일 리마인더 알림, 친구·채팅·카드 스냅샷 공유, JWT 인증을 구현하고, SwiftUI 구조 전면 재설계 및 Mock 기능의 실서버 전환을 완수했습니다.`,
    problem: `현대 스마트폰 사용자는 유용한 정보를 스크린샷으로 자주 저장하지만, 사진첩에 수천 장의 사진과 뒤섞여 정작 필요한 순간에 검색하거나 찾기 어려워 '디지털 무덤'으로 방치됩니다. 또한 **금융 정보나 개인정보가 포함된 스크린샷을 외부 클라우드 AI 서버로 무분별하게 전송할 경우 심각한 프라이버시 침해 위험**이 발생하는 문제가 있었습니다.`,
    troubleshooting: `Apple Vision Framework를 활용해 기기 내부(On-Device)에서 1차로 텍스트를 인식하고 정규표현식 기반으로 민감 개인정보를 마스킹한 뒤, **필요한 텍스트 메타데이터만 Spring Boot 백엔드로 비동기 전송하는 하이브리드 파이프라인**을 구축했습니다. 이를 통해 사용자 프라이버시를 원천 보호하면서도 서버 트래픽과 연산 비용을 대폭 절감하고 빠른 응답 속도를 확보했습니다.`,
  ...
  "Ticker — Human Stock Market": {
    summary: `개인의 가치, 성장 지표, 성과를 **주식 시장 메커니즘으로 시각화하고 상호 투자·응원할 수 있는 소셜 파이낸스 플랫폼**입니다. 실시간 호가 변동과 가상 주식 거래 시스템을 웹으로 구현했습니다.`,
    ...
    problem: `여러 사용자가 동시에 특정 인물의 가상 주식을 매수하거나 매도할 때 **데이터베이스 동시성 이슈(Race Condition)가 발생해 잔고가 음수가 되거나 체결가가 왜곡되는 데이터 불일치 위험**이 있었습니다.`,
    troubleshooting: `데이터베이스 **비관적 락(Pessimistic Lock)과 트랜잭션 격리 수준을 도메인 특성에 맞게 강화**하고, 주문 체결 전 잔고 검증 및 주문 큐 로직을 설계하여 동시 주문 상황에서도 잔고와 주가 정합성을 100% 보장했습니다.`,
  ...
  "Love Algorithm — 알고리즘보다 어려운 건 사랑이었다": {
    ...
    role: `팀원들과 함께 스토리텔링과 복잡한 선택지 분기 구조를 설계하고, **백엔드 세션 API, 진행 상태 저장 로직, 12가지 분기별 멀티 엔딩 계산 엔진**을 구현했습니다.`,
    problem: `사용자의 다양한 선택에 따라 수많은 스토리 분기와 캐릭터 호감도 수치가 실시간으로 갱신되는데, 새로고침이나 네트워크 지연 시 **세션 상태가 유실되어 엉뚱한 엔딩으로 이어지는 문제**가 있었습니다.`,
  ...
  "EGGO — 농꾸하고 작심삼일 타파하자": {
    ...
    role: `팀원과 함께 기획 및 UI/UX를 정리하고, **Flutter/Dart 기반 크로스플랫폼 모바일 앱 개발, Google ML Kit 온디바이스 비전 모델 연동 및 Gemini AI API 검증 파이프라인**을 전담했습니다.`,
  ...
  "자율 추종 스마트 카트 — Aruco·LiDAR 센서 융합": {
    summary: `**Aruco 비전 마커와 2D LiDAR 센서를 결합해 사용자를 인식하고 안전 거리를 유지하며 스스로 따라가는 스마트 카트**를 ROS2와 Gazebo 가상 환경에서 구현한 1인 로보틱스 프로젝트입니다.`,
    role: `ROS2 가상 주행 환경 구축부터 센서 데이터 필터링, **OpenCV Aruco 마커 검출, P-control 로봇 제어, LiDAR 긴급 정지 안전 알고리즘, Gazebo Sim 통신 브릿지 디버깅, 데모 영상 제작**까지 전 과정을 1인으로 수행했습니다.`,
    problem: `카메라만 사용하면 대상을 놓치거나 조명 변화 시 돌발 장애물을 감지하기 어려웠고, 최신 Gazebo Sim 환경에서 메시지 규격 차이로 인해 **주행 명령(/cmd_vel)이 로봇까지 전달되지 않는 통신 문제**가 있었습니다.`,
    troubleshooting: `OpenCV Aruco 마커의 오차를 이용해 P-control로 속도를 제어하고, **LiDAR로 0.5m 이하 장애물 감지 시 모든 주행 명령을 강제로 0으로 덮어쓰는 긴급 정지 로직**을 구현했습니다. Gazebo Sim 통신 문제는 ros_gz_bridge로 Twist 규격을 명시적으로 중계해 해결했습니다.`,
  ...
  "Localhost — 멀티플레이어 뮤직 퀴즈 게임": {
    summary: `여러 사용자가 웹 브라우저를 통해 실시간으로 방에 접속해 **음악을 함께 듣고 퀴즈를 맞히는 멀티플레이어 실시간 웹 게임**입니다.`,
    role: `팀원과 함께 기획 및 UI/UX를 정리하고, **웹소켓(WebSocket/Socket.io) 기반 실시간 동기화 프론트엔드 UI/UX, 반응형 게임 애니메이션 및 오디오 스트리밍 플레이어**를 구현했습니다.`,
    problem: `네트워크 지연으로 인해 사용자 간 **음악 재생 타이밍과 정답 제출 순서의 불일치**가 발생해 공정한 게임 진행이 어려운 문제가 있었습니다.`,
    troubleshooting: `서버 기준 타임스탬프 동기화 및 **지연 시간 보정(Latency Compensation) 알고리즘**을 프론트엔드에 구현해 모든 플레이어의 동기화 오차를 최소화했습니다.`,
    result: `**WebSocket 기반 실시간 동기화 게임 화면, 애니메이션 효과 및 오디오 재생 흐름**을 구현하여 여러 사용자가 동시 접속해 즐길 수 있는 음악 퀴즈 게임을 완성했습니다.`,
  ...
  "도다(DODA) — 정보 장벽을 낮추는 콘텐츠 플랫폼": {
    ...
    role: `3일 해커톤에서 기획자·디자이너·프론트엔드·백엔드 팀원들과 협업하여 **Java 17·Spring Boot 기반 REST API 백엔드 설계, 회원가입 및 JWT 인증, 복지 정보 CRUD·필터링·북마크, Apache POI Excel 뉴스 일괄 등록 및 AI 요약 API 구현, Docker/AWS EC2 배포 파이프라인 구축**을 전담했습니다.`,
    problem: `72시간이라는 극도로 짧은 해커톤 시간 안에 **복잡한 복지 데이터 모델을 설계하고, 대량의 엑셀 뉴스 데이터를 파싱해 요약 카드로 변환하는 백엔드 API와 클라우드 실서버 배포**까지 완수해야 했습니다.`,
    troubleshooting: `Spring Boot 도메인 기반 모듈화 설계와 Apache POI 엑셀 일괄 파싱 파이프라인을 구축하고, **Docker와 GitHub Actions를 결합한 CI/CD를 구성**해 해커톤 기간 중에도 프론트엔드 팀원이 즉시 연동 가능한 라이브 HTTPS API 환경을 제공했습니다.`,
