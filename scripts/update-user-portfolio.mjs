...
const userId = "4573257f-a9af-46ef-b1ec-f5593be14b1c";
...
협업할 때는 의견을 자유롭게 내는 만큼 **다른 사람의 의견을 경청하고 조율하는 것**을 중요하게 생각합니다. **초3부터 고2까지의 연속 임원과 동아리 회장 경험**을 바탕으로, 일정이나 역할에 문제가 생기면 **감정과 사실을 분리해 따뜻하지만 명확하게 소통**하며 함께 해결할 방법을 찾습니다.`;
...
협업할 때는 **각자의 진행 상황을 투명하게 공유**하고, 병목이나 이슈가 생기면 **감정에 치우치지 않고 사실을 중심으로 빠르게 공유**해 함께 대안을 모색합니다.`;
...
새로운 기술이나 언어를 배울 때는 **AI를 적극 활용해 학습 속도를 극대화**하고, 직접 코드를 작성하고 검증하며 단단한 제 것으로 체화하겠습니다. 백엔드 개발을 중심으로 성장하면서도 **사용자가 실제로 마주하는 서비스 경험과 전체 시스템 품질을 함께 책임지는 개발자**로 성장하고자 합니다.`;
...
const projectUpdates = {
  ...
  "CapLog": {
    ...
    next_time: "**위치 기반 스마트 알림 알고리즘**을 더욱 정교화하여 특정 장소 방문 시 관련 캡처를 추천하는 기능 고도화 예정",
  },
  ...
  "자율 추종 스마트 카트 — Aruco·LiDAR 센서 융합": {
    ...
    result: "**카메라·LiDAR 인식, 판단, /cmd_vel 구동이 매끄럽게 이어지는 자율 추종 시뮬레이션을 완성**했습니다. 대상 추종 중 전방 장애물이 감지되면 **비전 추종보다 긴급 정지가 우선하는 2중 안전 동작을 완벽히 검증**했습니다.",
...
console.log(`Updating ${projects.rows.length} projects in DB...`);
...
for (const row of projects.rows) {
  ...
  if (patch) {
    await client.query(
      `UPDATE projects
          SET summary = $1,
              ...
              troubleshooting = $4,
              result = $5,
              target_audience = COALESCE(NULLIF($6, ''), target_audience),
              goal = COALESCE(NULLIF($7, ''), goal),
              constraints = COALESCE(NULLIF($8, ''), constraints),
              key_decision = COALESCE(NULLIF($9, ''), key_decision),
              collaboration = COALESCE(NULLIF($10, ''), collaboration),
              architecture = COALESCE(NULLIF($11, ''), architecture),
              quality_assurance = COALESCE(NULLIF($12, ''), quality_assurance),
              deployment = COALESCE(NULLIF($13, ''), deployment),
              evidence = COALESCE(NULLIF($14, ''), evidence),
              learnings = COALESCE(NULLIF($15, ''), learnings),
              next_time = COALESCE(NULLIF($16, ''), next_time)
        WHERE id = $17`,
      [
        patch.summary,
        patch.role,
    ...
    console.log("Updated project in DB:", row.title);
  }
}

await client.end();
console.log("DB update for 배서연 (alicebsy@naver.com) completed successfully!");
