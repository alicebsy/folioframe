...
const projectsData = {
  ...
  "Ticker — Human Stock Market": {
    "summary": "개인의 생산성과 성취를 실시간 자산 가치(주가)로 평가하고 상호 투자·응원할 수 있는 인적 자본 시장(Human Stock Market) 플랫폼입니다. '가장 높은 가치는 당신의 오늘로부터'라는 슬로건 아래 기록형 To-Do에 자본주의적 피드백 시스템을 결합한 Social-Fi 생태계를 지향합니다.",
    "role": "팀원들과 함께 서비스 기획 및 프론트엔드 연동 회의를 주도하고, Spring Boot (Java 17) 기반 백엔드 아키텍처 설계, STOMP WebSocket 기반 실시간 호가/주가 변동 전파 파이프라인, 가상 주식 매수·매도 거래 체결 엔진, 트랜잭션 격리 수준 및 데이터 무결성 보장, 카카오 OAuth2 및 JWT 인증 RESTful API 구현 전반을 전담했습니다.",
    "problem": "기존 자기계발 앱은 실패해도 리스크가 없고 보상이 추상적이어서 장기적 동기부여가 어려운 한계가 있었습니다. 이를 해결하기 위해 성취도 기반 실시간 주가 산정 모델을 구축해야 했으며, 다수의 사용자가 동시에 특정 인물의 주식을 매수하거나 매도할 때 동시성 이슈(Race Condition)로 인해 잔고 왜곡이나 체결 가격 불일치가 발생할 위험이 있었습니다.",
    "troubleshooting": "Spring Boot 트랜잭션 격리 수준을 최적화하고 비관적 락(Pessimistic Lock)과 원자적 잔고 검증 로직을 적용해 동시 다발적인 매수·매도 주문 상황에서도 데이터 정합성을 100% 보장했습니다. 또한 STOMP WebSocket 프로토콜을 통해 호가창과 주가 변동 이벤트를 밀리초 단위로 브로드캐스팅하는 고반응성 이벤트 브로커 구조를 완성했습니다.",
    "result": "실시간 주가 차트, 투두 완료 기반 주가 반영 알고리즘, 가상 주식 매수/매도 체결 엔진, 총자산 및 투자 평가액 포트폴리오, 랭킹 및 암시장 시스템을 완벽히 구현하여 실제 macOS/iOS 클라이언트와 백엔드 간 무결점 실시간 거래 환경을 성공적으로 시연했습니다.",
  ...
  "Love Algorithm — 알고리즘보다 어려운 건 사랑이었다": {
    "summary": "알고리즘 문제 해결 상황과 개발자 밈을 인터랙티브 스토리텔링과 연애 시뮬레이션 비주얼 노벨 게임으로 유쾌하게 풀어낸 웹 콘텐츠입니다.",
    "role": "팀원들과 함께 시나리오와 복잡한 선택지 분기 트리를 설계하고, React + TypeScript + Vite 기반 프론트엔드 인터랙션과 Spring Boot 백엔드 세션 API, 진행 상태 영속화 로직, 12가지 분기별 멀티 엔딩 판별 계산 엔진을 구현했습니다.",
    "problem": "사용자의 다양한 선택에 따라 방대하게 분기되는 상태 트리 속에서 세션 이탈 시 데이터 유실 없이 정확한 시나리오 노드를 복원해야 했고, 프론트엔드 모킹 모드(Mocking)와 실제 백엔드 연동 모드를 유연하게 전환할 수 있어야 했습니다.",
    "troubleshooting": "유한 상태 머신(Finite State Machine) 패턴을 기반으로 씬(Scene) 전이 로직을 모델링하여 분기 전환 비용을 최소화하고 상태 복구 안정성을 극대화했습니다. 또한 환경 변수 기반 API 클라이언트 어댑터 패턴을 적용해 백엔드 유무에 관계없이 독립적인 개발 및 검증이 가능하도록 설계했습니다.",
    ...
    "architecture": "React 18 + TypeScript + Vite + CSS Modules / Java 17 + Spring Boot + REST API + Vercel & 클라우드 배포",
    "quality_assurance": "12가지 분기 루트 전수 시나리오 테스트, 세션 로컬 스토리지 및 DB 복구 검증",
    "deployment": "Vercel 프론트엔드 자동 배포 및 Spring Boot 백엔드 라이브 배포 (lovealgorithmgame.site)",
  ...
  "Localhost — 멀티플레이어 뮤직 퀴즈 게임": {
    "summary": "여러 사용자가 웹 브라우저를 통해 실시간으로 방에 접속해 노래 맞추기, 사투리 가사 퀴즈 등을 함께 플레이하는 3D 인터랙티브 멀티플레이어 실시간 웹 게임입니다. 'Local(지역·사투리) + Host(주최자·DJ)'이자 개발자들에게 친숙한 'localhost'라는 의미를 담아 기획되었습니다.",
